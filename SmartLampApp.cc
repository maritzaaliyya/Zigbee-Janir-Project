...
Define_Module(SmartLampApp);

SmartLampApp::~SmartLampApp()
{
    cancelAndDelete(selfMsg);
}

void SmartLampApp::initialize(int stage)
{
...
void SmartLampApp::setSocketOptions()
{
...
L3Address SmartLampApp::chooseDestAddr()
{
...
void SmartLampApp::sendPacket()
{
    ...
    payload->addTag<LampStateTag>()->setIsOn(lampOn);
...
void SmartLampApp::processStart()
{
...
void SmartLampApp::processStop()
{
    socket.close();
}

void SmartLampApp::handleMessageWhenUp(cMessage *msg)
{
...
void SmartLampApp::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    processPacket(packet);
}

void SmartLampApp::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void SmartLampApp::socketClosed(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void SmartLampApp::refreshDisplay() const
{
...
void SmartLampApp::processPacket(Packet *pk)
{
...
void SmartLampApp::handleStartOperation(LifecycleOperation *operation)
{
...
void SmartLampApp::handleStopOperation(LifecycleOperation *operation)
{
    cancelEvent(selfMsg);
...
void SmartLampApp::handleCrashOperation(LifecycleOperation *operation)
{
...
void SmartLampApp::scheduleOnOffCheck()
{
...
void SmartLampApp::checkOnOffTime()
{
    ...
    if (shouldBeOn && !lampOn) {
          EV_INFO << "Turning lamp ON at " <<  std::put_time(now_tm, "%H:%M:%S") << "\n";

        lampOn = true;

    } else if (!shouldBeOn && lampOn) {
        EV_INFO << "Turning lamp OFF at " << std::put_time(now_tm, "%H:%M:%S") << "\n";
